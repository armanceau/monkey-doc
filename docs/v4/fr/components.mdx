...
Un lien stylisé en bouton. Trois variantes et trois tailles — idéal pour les CTA, liens de téléchargement ou la navigation.
...
<div className="flex flex-wrap gap-3 my-4">
  <LinkButton href="/fr/getting-started" size="sm">Petit</LinkButton>
  <LinkButton href="/fr/getting-started" size="md">Moyen</LinkButton>
  <LinkButton href="/fr/getting-started" size="lg">Grand</LinkButton>
</div>
...
Props : `href` · `variant` (`default` / `outline` / `ghost`) · `size` (`sm` / `md` / `lg`) · `external` (détecté automatiquement depuis l'URL)
...
<BarChart
  labels={["Jan", "Fév", "Mar"]}
  datasets={[
    { label: "Vues", data: [4200, 5800, 4900] },
    { label: "Visiteurs", data: [2100, 3200, 2700] }
  ]}
  title="Trafic mensuel"
/>
```

Ajoutez `horizontal` pour un affichage horizontal. Utilisez `height` pour ajuster la hauteur (défaut `280`).
